...
import com.example.myapplication2.databinding.ItemPopularBinding

class PopularAdapter(
    private val clickControl: (Long) -> Unit

) : RecyclerView.Adapter<PopularAdapter.MyViewHolder>() {
    class MyViewHolder(var binding: ItemPopularBinding) : ViewHolder(binding.root)
    ...
    var popularList: List<MovieBriefInfoModel>
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        holder.binding.data = popularList[position]
        holder.binding.date = popularList[position].release_date?.substring(0, 4)
        holder.binding.rating = popularList[position].vote_average.toString()
        holder.binding.cardPop.setOnClickListener {
            popularList[position].id?.let { selectedMovie -> clickControl(selectedMovie) }
        }
    }
